...
class DepthAnything3(nn.Module, PyTorchModelHubMixin):
    ...
    def inference(
        ...
        # CAUTION: Changed to fit the predictions
        # ex_t_norm = ex_t
        ...
        if render_exts is not None and render_ixts is not None and scale is not None:
            render_exts[..., :3, 3] = render_exts[..., :3, 3] * scale
            new_w, new_h = imgs_cpu.shape[3], imgs_cpu.shape[2]
            w, h = render_ixts[0, 0, 2] * 2, render_ixts[0, 1, 2] * 2
            ...
            render_exts = torch.from_numpy(render_exts).float().to(self.device).unsqueeze(0)
            render_ixts = torch.from_numpy(render_ixts).float().to(self.device).unsqueeze(0)
    ...
    def _normalize_extrinsics(self, ex_t: torch.Tensor | None) -> torch.Tensor | None:
        ...
        print(f"Median camera distance for normalization: {median_dist.item()}")
    ...
    def _align_to_input_extrinsics_intrinsics(
        ...
        print(f"Estimated scale factor: {scale}")
        visualize_cameras_plotly(prediction.extrinsics, aligned_extrinsics)
