    ...
    def _process_one(
        ...
        *,
        ...
        if process_res_method.endswith("resize"):
            # MONKEY PATCH: resize to nearest multiple of PATCH_SIZE and 8, which is PATCH_SIZE*4
            pil_img = self._make_divisible_by_resize(pil_img, self.PATCH_SIZE * 4)
            new_w, new_h = pil_img.size
            intrinsic = self._resize_ixt(intrinsic, w, h, new_w, new_h)
            w, h = new_w, new_h
        elif process_res_method.endswith("crop"):
            pil_img = self._make_divisible_by_crop(pil_img, self.PATCH_SIZE * 4)
            new_w, new_h = pil_img.size
