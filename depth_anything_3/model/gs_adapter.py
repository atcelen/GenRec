...
from depth_anything_3.utils.visualize import visualize_cameras_plotly
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.pred_offset_depth:
            raw_offset = raw_gaussians[..., -1]
            # Offset too large/small depths can destabilize Gaussian renders
            # print("Absolute depth offset min/max:", raw_offset.min().item(), raw_offset.max().item())
            # print("Absolute depth min/max:", depths.min().item(), depths.max().item())
            # raw_offset = torch.clamp(raw_offset, min=-0.5 * depths, max=0.5 * depths)
            gs_depths = depths + raw_offset
        ...
        if gt_extrinsics is not None and not torch.equal(extrinsics, gt_extrinsics):
            try:
                ...
                raise Exception("Force using default scale 1.0")
            except Exception:
                pose_scales = torch.ones_like(extrinsics[:, 0, 0, 0])
            
            # print(f"Pose Scale: {pose_scales}")
            # visualize_cameras_plotly(gt_extrinsics[0].cpu(), extrinsics[0].cpu())   
            # rescaled_gt_extrinsics = gt_extrinsics.clone().cpu()
            # rescaled_gt_extrinsics[:, :, :3, 3] = rescaled_gt_extrinsics[:, :, :3, 3] / rearrange(
            #     pose_scales.cpu(), "b -> b () ()"
            # )
            # visualize_cameras_plotly(rescaled_gt_extrinsics[0].cpu(), extrinsics[0].cpu())
