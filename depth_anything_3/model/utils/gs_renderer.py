...
def render_3dgs(
    ...
    **kwargs,
    ...
    for i in range(batch_scene):
        ...
        noise = torch.randn_like(render_colors[..., :3])
        noise = (noise * 0.5 + 0.5).clamp(0.0, 1.0)

        # render_colors[..., :3] = render_colors[..., :3] + noise * (1.0 - render_alphas) ** 3
        # render_colors[..., :3] = render_colors[..., :3] + noise * 0.5 * (1.0 + torch.cos(render_alphas * math.pi))

        gaussian_2d_sizes, _ = torch.max(info["radii"], dim=-1, keepdim=True)
        gaussian_2d_sizes = gaussian_2d_sizes.float()
        # radius_bg = torch.full((num_view, 1), 1000.0, device=i_backgrounds.device)
        # radius_bg = torch.zeros((num_view, 1), device=i_backgrounds.device)
        radius_bg, _ = torch.max(gaussian_2d_sizes, dim=1)

        blur_map, _, _ = rasterization(
            means=i_means,
            quats=i_quats,
            scales=i_scales,
            opacities=i_opacities,
            colors=gaussian_2d_sizes,
            viewmats=i_viewmats,
            Ks=K,
            backgrounds=radius_bg,
            render_mode="RGB",       
            width=w,
            height=h,
            packed=False,
        ...
        try:
            ...
            pass
        alpha = render_alphas.unbind(dim=0)
        blur_map = blur_map.unbind(dim=0)
        ...
        all_blur_maps.extend(blur_map)

    return torch.stack(all_images), torch.stack(all_depths), torch.stack(all_alphas), torch.stack(all_blur_maps)
...
def run_renderer_in_chunk_w_trj_mode(
    ...
    **kwargs,
    ...
    for chunk_idx in tqdm(
        ...
        color, depth, alpha, blur_map = render_3dgs(
        ...
        all_alphas.append(rearrange(alpha, "(b v) ... -> b v ...", v=cur_n_view))
        all_blur_maps.append(rearrange(blur_map, "(b v) ... -> b v ...", v=cur_n_view))
    all_colors = torch.cat(all_colors, dim=1)
    all_depths = torch.cat(all_depths, dim=1)
    all_alphas = torch.cat(all_alphas, dim=1)
    all_blur_maps = torch.cat(all_blur_maps, dim=1)

    return all_colors, all_depths, all_alphas, all_blur_maps
