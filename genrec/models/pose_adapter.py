...
from diffusers.models.attention_processor import Attention, XFormersAttnProcessor, AttnProcessor2_0
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                ...
                Attention(
                        ...
                        processor=AttnProcessor2_0(), #XFormersAttnProcessor(),
                    ),
...
class RayMapEncoder(ModelMixin, ConfigMixin):
    ...
    def __init__(self, 
        ...
        # Added for torch.compile compatibility
        pos_embed_tensor = posemb_sincos_2d(
            h = self.patched_h,
            w = self.patched_w,
            dim = self.patched_embed_dim,
        )

        self.register_buffer("pos_embedding", pos_embed_tensor)

    # Removed for torch.compile compatibility
    # @property
    ...
    def from_pretrained_new(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        # Load the ray-encoder weights from either a local directory or a
        # HuggingFace repo subfolder (auto-downloaded + cached).
        subfolder_name = "ray_encoder"
        weight_filenames = [
            "diffusion_pytorch_model.safetensors",
            "model.safetensors",
            "pytorch_model.bin",
        ]
        model_dir = os.path.join(pretrained_model_name_or_path, subfolder_name)
        print(f"Loading ray encoder weights from {model_dir}")
        ...
        if os.path.isdir(model_dir):
            for fname in weight_filenames:
                ...
                if os.path.exists(fpath):
                    state_dict = _load_state_dict_from_file(fpath)
                    break
            if state_dict is None:
                # Config-only directory (e.g. the vendored genrec/model_config):
                # the ray-encoder weights are supplied by the full GenRec
                # checkpoint loaded afterwards, so random init here is fine.
                print(
                    f"No ray_encoder weights in {model_dir}; using random init "
                    f"(weights are expected to come from the GenRec checkpoint)."
                )
                return model
        else:
            # Treat pretrained_model_name_or_path as a HuggingFace repo id.
            ...
            if state_dict is None:
                raise FileNotFoundError(
                    f"No ray_encoder weights found in HuggingFace repo "
                    f"'{pretrained_model_name_or_path}' (subfolder '{subfolder_name}'). "
                    f"Expected one of {weight_filenames}."
                )
    ...
    def forward(self, x: Float[Tensor, "B T C H W"], **kwargs) -> Float[Tensor, "B T C H W"]:
        ...
        p_h, p_w = self.config.patch_size, self.config.patch_size
        patched_h = h // p_h
        patched_w = w // p_w
        ...
        if not hasattr(self, "_cached_pos_embed") or self._cached_pos_embed.shape[0] != x.shape[1]:
            self._cached_pos_embed = posemb_sincos_2d(
                patched_h, patched_w, self.patched_embed_dim, dtype=x.dtype
            ).to(x.device)

        x = x + self._cached_pos_embed
        
        # Changed for torch.compile compatibility
        # x = x + self.pos_embedding.to(dtype=x.dtype)

        x = self.transformer(x)
        
        output = self.linear_head(x)
        output = rearrange(output, 'b (h w) c -> b c h w', h=patched_h, w=patched_w)
